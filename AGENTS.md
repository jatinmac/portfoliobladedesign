...
- `ContentDrivenPageExperience.tsx`: generic Blade page renderer for frontmatter-backed content pages, including linked list rows and optional hero/action areas.
- `ProjectImageCarousel.tsx`: Blade `Carousel`/`CarouselItem` media for project pages with multiple images.
- `PageTransition.tsx`, `ScrollReveal.tsx`, and `StaggeredEntrance.tsx`: Framer Motion helpers that respect `useReducedMotion()`.
...
- `/about` renders `ContentDrivenPageExperience` with structured profile/about content from `content/about.md`, a LinkedIn action when available, and `assets/profile/about.png` as the hero image.
...
- `/projects` renders selected-work cards from `content/projects/*.md` with page copy from `content/projects.md` and hero artwork resolved through `resolveProjectHeroImage()`.
- `/projects/[slug]` renders a pure markdown case-study layout with metadata, content-backed product links, hero media, `ProjectImageCarousel` when `galleryImages` is configured, a `Pitch Deck` carousel when `pitchDeckImages` is configured, and structured markdown sections. It does not render `ChatPanel`.
...
- The `PortfolioSidebar` recent chat messages use direct `ActionList`/`ActionListItem` children and the existing `onSelectChat` routing/session behavior. The sidebar also owns collapsed/open desktop behavior, the mobile close variant, top projects, pages filtered to exclude `/resume`, and Contact/Resume link buttons.
- `PortfolioAppShell` is a behavior-preservation boundary for `sessionStorage` recent chats, active chat ID, first-message registration, new-chat routing, selected-chat routing, the mobile top bar/sidebar overlay, and page transitions through `PageTransition`.
...
- `ChatPanel` also owns the profile-card empty state, animated placeholder suggestions, Tab-to-accept placeholder behavior, retry/stop controls, response animation, and `sessionStorage` transcript persistence.
...
- Use `ContentDrivenPageExperience` for content-backed page layouts. It supports parsed `##` sections, `###` subsections, paragraphs, ordered/unordered lists, linked list rows, optional header actions, optional hero content, and scroll/stagger reveals.
