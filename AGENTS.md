...
- `HomepageChatExperience.tsx`: persistent right-column chat composition around portfolio-scoped `ChatPanel`, delegating the empty-state carousel to the lazy-loaded `HomepageProjectCarousel.tsx`.
...
- `/experience`, `/contact`, and `/projects` also render in the left pane of the split workspace, with the persistent portfolio AI chat on the right. Project detail routes (`/projects/[slug]`) remain full-width content routes and do not render `ChatPanel`.
...
- `PortfolioAppShell` is a behavior-preservation boundary for `sessionStorage` recent chats, active chat ID, first-message registration, new-chat routing, selected-chat routing, the mobile top bar (displaying dynamic page titles via `getPageTitle()`), mobile sidebar layout (flex display toggled directly rather than using Framer Motion animations), split workspace routing for `/`, `/about`, `/experience`, `/contact`, and `/projects`, and scrolling progress monitoring through `ScrollProgressBar` on non-homepage pages. The desktop split currently keeps the right chat column capped at `500px` on `l` and `520px` on `xl`; do not widen that baseline unless a new design explicitly asks for it.
